...
public class MainClass {
	...
	public static void main(String[] args) {
		 ...
		 * &query=%EC%9E%90%EB%B0%94 CheckException : JVM이 감시 (에외처리를 했는지?)
		 * ============== **네트워크 (URL,서버 IP) => 수정이 가능한 에러 java.net
		 * 
		 * ***= URL (웹에 연결) ***= URLEncoding() => 1byte => 2byte 인코딩 디코딩 자바 => byte[] =>
		 * %EC%9E%90%EB%B0%94(인코딩) btye[] => 자바(char) => 자바 (디코딩) 문자열을 바이트 배열로 변경 ==>
		 * "".getBytes() = ServerSocket : 대기소켓(접속시 처리) = Socket: 통신 소켓 (서버<==>클라이언트)
		 * **파일 (경로명, 파일명) => 수정이 가능한 에러 java.io = File = FileInputStream /
		 * FileOutputStream(1byte) => 일반 파일 사용시 = FileReader / FileWriter(2byte) => 한글이
		 * 포함된 파일을 읽는 경우 = 속도 최적화 BufferedReader 쓰레드 (동시 실행 => 충돌) java.lang **오라클 연결
		 * (SQL문장) java.sql = Connection : 오라클 연결 = Statement : 오라클 명령어 전송 (SQL) =
		 * ResultSet : 결과값을 받는다
		 * 
		 * => 예외 복구 => 정상 수행 (직접 처리) => *** try { 정상 수행 }catch(예외 처리 종류) { 에러 발생시 처리
		 * (확인) } finally { 서버닫기 , 오라클 연결 해제, 파일 닫기 ... } => 예외 회피 => 정상 수행 (시스템에 맞긴다)
		 * method() throws 예외처리 ....
		 */
		try {
			...
			// 연결
			/*
			 * URLConnection (file에 접근이 가능) file:///c://javaDev//a.jpg | HttpURLConnection
			 * 
