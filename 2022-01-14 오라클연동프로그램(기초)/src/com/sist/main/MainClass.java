...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		/*
		 * // 요청후 데이터를 얻어 온다 => empListData() List<Emp> list=dao.empListData(); // 사용자
		 * => 데이터 출력 for(Emp e:list) { System.out.println(e.getEmpno()+" "
		 * +e.getEname()+" " +e.getJob()+" " +e.getHiredate().toString()+" "
		 * +e.getSal()); }
		 */
		/*
		 * Scanner scan=new Scanner(System.in); System.out.print("사번입력:"); int
		 * empno=scan.nextInt(); // Front(뷰단) , Back(서버단) , DBA (DB단) Emp
		 * emp=dao.empDetailData(empno); // 요청 처리 (메소드 안에서) ==> 결과값을 전송 (Emp:한명에 대한 정보를
		 * 가지고 있다) // 모든 클래스의 설계는 1개,한명을 설계한다 ==> 여러개,여러명 : new를 여러번 사용 // 사용자에게 출력
