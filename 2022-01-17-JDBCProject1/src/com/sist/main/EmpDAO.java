...
public class EmpDAO {
	...
	/*
	 * 기본 사이트 : 로그인 , 회원가입(아이디 중복,아이디 찾기 , 우편번호 검색) 공지사항
	 */
	// 기능
	...
	public List<DeptVO> deptListData() {
		 ...
		 * | ArrayList Vector Queue => List를 구현하고 있는 클래스 | LinkedList => 저장을 할때 =>
		 * Object => 원하는 데이터형으로 변경 : 제네릭스
		 */
		try {
			// 1. 연결
			getConnection();
	...
	/*
	 * 화면 출력 ------- 1. 목록 ==> List 2. 목록에서 한개를 클릭 => 상세보기 ==> ~VO 3. 일반 변수 =>
	 * boolean(로그인처리, 아이디 중복) , int (총페이지) 4. insert , update ,delete => 오라클 자체 처리
	 * => void 5. 제목만 , 장르만 , 가수만 , 출연만 .... List<String>
	 * 
	 * ------------------------------------------------------ MovieVO => 영화 1개에 대한
	 * 정보를 가지고 있다 => List<MovieVO> : 영화 여러개를 묶는다 => 극장명 (날짜 => 시간 => 좌석) EmpVO => 사원
	 * 1명에 대한 정보 => 부서 , 등급 MusicVO => 음악 1개에 대한 정보 FoodVO => 맛집 1개에 대한 정보 RecipeVO
	 * => 레시피 1개에 대한 정보 ------------ 해당 테이블의 컬럼명과 일치 + 추가가 가능 ---------
	 * 
	 * 자바 <=======> 오라클 1. 오라클에 있는 데이터를 자바에서 받는다 오라클 (컬럼) = 자바(멤버변수) 2. 여러개를 가지고 올때는
	 * List에 묶어서 처리
	 */
	...
	public List<EmpVO> empSubQueryData2() {
		...
		try {
			...
			// 2. SQL문장
			/*
			 * String sql="SELECT ROUND(AVG(sal)) FROM emp"; // 3. 결과값 받기
	...
	// 인라인뷰
	/*
	 * SELECT : 사용 1) 전체 목록 (목록 출력) 2) 조건 => 1개 (상세보기) 3) 조인 4) 서브쿼리 : 1) 스칼라 , 2)
	 * 인라인뷰
	 */
